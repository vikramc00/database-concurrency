...
public class Database implements AutoCloseable {
    ...
    private class TransactionContextImpl extends TransactionContext {
        ...
        public void close() {
            try {
                // TODO(proj4_part2)
                LockContext firstC = null;

                List<Lock> lockList = Database.this.lockManager.getLocks(this);

                List<LockContext> track = new ArrayList<>();
                Set<LockContext> contextsList = new HashSet<>();
                Stack<LockContext> oN = new Stack<>();
                Queue<LockContext> f = new ArrayDeque<>();
                ...
                for (int i=0; i<lockList.size(); i++) {
                    LockContext thisContext = LockContext.fromResourceName(Database.this.lockManager, lockList.get(i).name);
                    contextsList.add(thisContext);
                    if (thisContext.parentContext() == null) {
                        firstC = thisContext;
                    }
                }
                f.add(firstC);
                ...
                while (!f.isEmpty()) {
                    LockContext thisContext = f.poll();
                    track.add(thisContext);
                    oN.push(thisContext);
                    for (LockContext l: contextsList) {
                        if ( thisContext == l.parentContext() && !track.contains(l) ) {
                            f.add(l);
                        }
                    }
                }

                while (oN.isEmpty() != true) {
                    LockContext finish = oN.pop();
                    finish.release(this);
                }
                return;
